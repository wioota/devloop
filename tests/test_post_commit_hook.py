...
HOOK_TEMPLATE_PATH = Path("src/devloop/cli/templates/git_hooks/post-commit")
...
class TestPostCommitHook:
    ...
    def test_hook_exists_and_executable(self):
        """Test that post-commit hook template exists and is executable."""
        assert HOOK_TEMPLATE_PATH.exists(), "post-commit hook template does not exist"
        assert os.access(
            HOOK_TEMPLATE_PATH, os.X_OK
        ), "post-commit hook template is not executable"

    def test_hook_parses_fixes_keyword(self):
        """Test that hook recognizes 'fixes' keyword."""
        hook_content = HOOK_TEMPLATE_PATH.read_text()

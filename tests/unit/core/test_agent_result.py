...
class TestAgentResultValidCreation:
    ...
    def test_minimal_valid_creation(self):
        ...
        result = AgentResult(agent_name="test-agent", success=True, duration=1.5)
    ...
    def test_zero_duration(self):
        ...
        result = AgentResult(agent_name="test-agent", success=True, duration=0.0)

        assert result.duration == 0.0

    def test_integer_duration(self):
        """Test that integer duration is accepted."""
        result = AgentResult(agent_name="test-agent", success=True, duration=5)
    ...
    def test_empty_data_dict(self):
        ...
        result = AgentResult(
            agent_name="test-agent", success=True, duration=1.0, data={}
        )
...
class TestAgentResultInvalidCreation:
    ...
    def test_missing_duration_parameter(self):
        ...
        with pytest.raises(
            TypeError, match="missing 1 required positional argument: 'duration'"
        ):
            AgentResult(
                agent_name="test-agent",
                success=True,
    ...
    def test_invalid_agent_name_type(self):
        ...
        with pytest.raises(TypeError, match="agent_name must be a string"):
            AgentResult(agent_name=123, success=True, duration=1.0)  # Should be string
    ...
    def test_empty_agent_name(self):
        ...
        with pytest.raises(ValueError, match="agent_name cannot be empty"):
            AgentResult(agent_name="", success=True, duration=1.0)
    ...
    def test_invalid_success_type(self):
        ...
        with pytest.raises(TypeError, match="success must be a boolean"):
            AgentResult(
                agent_name="test-agent", success="yes", duration=1.0  # Should be bool
            )
    ...
    def test_invalid_duration_type_none(self):
        ...
        with pytest.raises(
            TypeError, match="duration must be a number.*Did you forget"
        ):
            AgentResult(
                agent_name="test-agent", success=True, duration=None  # Should be number
            )

    def test_negative_duration(self):
        """Test that negative duration raises ValueError."""
        with pytest.raises(ValueError, match="duration must be non-negative"):
            AgentResult(agent_name="test-agent", success=True, duration=-1.0)
...
class TestAgentResultEdgeCases:
    ...
    def test_very_long_agent_name(self):
        ...
        long_name = "a" * 1000
        result = AgentResult(agent_name=long_name, success=True, duration=1.0)
    ...
    def test_very_long_message(self):
        ...
        result = AgentResult(
            agent_name="test-agent", success=True, duration=1.0, message=long_message
        )
    ...
    def test_very_large_duration(self):
        ...
        result = AgentResult(agent_name="test-agent", success=True, duration=999999.999)

        assert result.duration == 999999.999

    def test_unicode_agent_name(self):
        """Test that unicode agent names are accepted."""
        result = AgentResult(agent_name="测试-агент-🤖", success=True, duration=1.0)
    ...
    def test_complex_nested_data(self):
        ...
        complex_data = {
            "level1": {"level2": {"level3": ["a", "b", {"level4": True}]}},
            "list": [1, 2, 3],
            "mixed": {"str": "value", "int": 42, "bool": False},
        }
        result = AgentResult(
            agent_name="test-agent", success=True, duration=1.0, data=complex_data
        )
...
Line 3: More details"""
        result = AgentResult(
            agent_name="test-agent", success=False, duration=1.0, error=multiline_error
        )

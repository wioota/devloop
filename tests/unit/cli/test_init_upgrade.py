...
class TestManifestHelpers:
    ...
    def test_read_missing_manifest_returns_defaults(self, claude_dir: Path) -> None:
    ...
    def test_write_and_read_manifest(self, claude_dir: Path) -> None:
        with patch("devloop.__version__", "1.0.0"):
            _write_init_manifest(claude_dir, ["a.txt", "b.txt"])
        manifest = _read_init_manifest(claude_dir)
        assert manifest["version"] == "1.0.0"
        assert manifest["managed"] == ["a.txt", "b.txt"]

    def test_write_manifest_stores_as_given(self, claude_dir: Path) -> None:
        with patch("devloop.__version__", "1.0.0"):
            _write_init_manifest(claude_dir, ["b.txt", "a.txt"])
        manifest = _read_init_manifest(claude_dir)
        assert manifest["managed"] == ["b.txt", "a.txt"]

    def test_read_corrupt_manifest_returns_defaults(self, claude_dir: Path) -> None:
    ...
    def test_needs_upgrade_different_version(self, claude_dir: Path) -> None:
        with patch("devloop.__version__", "0.8.0"):
            _write_init_manifest(claude_dir, [])
        with patch("devloop.__version__", "0.9.0"):
            assert _needs_upgrade(claude_dir) is True
...
class TestUpgradeRefresh:
    """Test that managed files are refreshed when content changes."""

    def test_hooks_overwritten_when_content_differs(self, project_dir: Path) -> None:
        ...
        # Tamper with a hook
        hook_name = managed_paths[0].split("/")[-1]
        hook_file = hooks_dir / hook_name
        hook_file.write_text("#!/bin/bash\n# user modified")

        # Re-run should detect content change, backup, and overwrite
        managed_again, updated_again = _create_claude_hooks(hooks_dir)
        assert managed_paths[0] in managed_again
    ...
    def test_hooks_skipped_when_content_identical(self, project_dir: Path) -> None:
        hooks_dir = project_dir / ".agents" / "hooks"
        hooks_dir.mkdir(parents=True)

        # First install
        managed_paths, _ = _create_claude_hooks(hooks_dir)
        assert len(managed_paths) > 0

        # Re-run with identical content: still returns all paths, 0 updated
        managed_again, updated_again = _create_claude_hooks(hooks_dir)
        assert managed_again == managed_paths
        assert updated_again == 0

    def test_commands_overwritten_when_content_differs(self, project_dir: Path) -> None:
        # First install
        managed_paths, _ = _setup_claude_commands(project_dir)
        if not managed_paths:
            pytest.skip("No template commands found")

        # Tamper with a command
        first_managed = project_dir / managed_paths[0]
        first_managed.write_text("# user modified")

        # Re-run should detect content change, backup, and overwrite
        managed_again, updated_again = _setup_claude_commands(project_dir)
        ...
        # Backup should exist
        backup = first_managed.with_suffix(".md.backup")
        assert backup.exists()
...
class TestStaleFileRemoval:
    ...
    def test_stale_files_removed(self, project_dir: Path, claude_dir: Path) -> None:
        ...
        with patch("devloop.__version__", "0.8.0"):
            _write_init_manifest(claude_dir, old_managed)
        ...
        old_set = set(old_manifest.get("managed", []))
    ...
    def test_user_files_not_removed(self, project_dir: Path, claude_dir: Path) -> None:
        ...
        with patch("devloop.__version__", "0.8.0"):
            _write_init_manifest(claude_dir, [".agents/hooks/managed-hook"])
...
class TestIdempotency:
    ...
    def test_manifest_unchanged_on_rerun(self, claude_dir: Path) -> None:
        ...
        managed = ["a.txt", "b.txt"]
        with patch("devloop.__version__", "1.0.0"):
            _write_init_manifest(claude_dir, managed)
    ...
    def test_hooks_not_rewritten_when_identical(self, project_dir: Path) -> None:
        """When content is identical, existing hooks are not touched."""
        hooks_dir = project_dir / ".agents" / "hooks"
        hooks_dir.mkdir(parents=True)
        ...
        _, updated_count = _create_claude_hooks(hooks_dir)
        assert updated_count == 0
